...
std::unique_ptr<BlockNode> Parser::parseBlock() {
    ...
    // Declaration section: declarations always start with a type keyword or PROCEDURE/CONSTANT
    while (true) {
...
ASTNodePtr Parser::parseStatement() {
    ...
    case TokenKind::TOK_IDENTIFIER:
        return parseAssignOrProcCall();
    case TokenKind::TOK_REGISTER: {
        // Register assignment: R0 := expr
        int regNum = cur_.regNum;
        SourceLoc regLoc = cur_.loc;
        advance();
        auto target = std::make_unique<RegisterNode>(regNum, regLoc);
...
ASTNodePtr Parser::parseAssignOrProcCall() {
    SourceLoc loc = cur_.loc;
    std::string name = expectIdentifier();
    ...
    if (check(TokenKind::TOK_COLON)) {
        ...
        return std::make_unique<LabelStmtNode>(name, std::move(stmt), loc);
    }

    // Build the LHS variable expression (may be array subscript)
    ASTNodePtr lhs = std::make_unique<IdentifierNode>(name, loc);

    if (check(TokenKind::TOK_LPAREN)) {
        advance();
        auto indices = parseArgList();
        expect(TokenKind::TOK_RPAREN);
        lhs = std::make_unique<IndexExprNode>(std::move(lhs), std::move(indices), loc);
    }

    // Dereference chain
    while (check(TokenKind::TOK_DEREF)) {
        advance();
        lhs = std::make_unique<DerefExprNode>(std::move(lhs), loc);
    }

    // Assignment
    expect(TokenKind::TOK_ASSIGN);
    auto value = parseExpression();
    return std::make_unique<AssignStmtNode>(std::move(lhs), std::move(value), loc);
}
